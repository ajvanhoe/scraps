	        ...
	        <div class="panel-heading">
	          <span style="font-size:16px;"> <strong>{{$category->title}}</strong></span>
	            ...
	            <button class="btn btn-outline btn-xs btn-danger" data-toggle="modal" data-target="#modal{{$category->id}}"><i class="fa fa-times"></i></button>
							...
							<form method="POST" action="{{route('delete.album.category', ['id' => $category->id])}}" id="delete:{{$category->id}}">
								{{ csrf_field() }} 
       				</form>
	        	</div>
	        </div>
...
<div class="panel-body">
    ...
    <div class="col-lg-6">
	    <ul class="list-group">
     	@foreach($subcategories as $subcategory)
          ...
          <dl>
            <dt>{{$subcategory->title}}<span style="float:right; font-size:10px;"><a href="{{route('delete.album.subcategory', ['id' => $subcategory->id])}}" onclick="confirm('ukloni?')">delete</a></span></dt>
            @foreach($subcategory->sub_subcats() as $subcat)
            <dd>
            - {{$subcat->title}}<span style="float:right; font-size:10px;"><a href="{{route('delete.album.subcategory', ['id' => $subcat->id])}}" onclick="confirm('ukloni?')">delete</a></span>
            </dd>
            @endforeach
  ...
  <div id="collapse1" class="collapse">
    <div class="panel-body">
      ...
      <form method="POST" action="{{route('edit.album.category', ['id' => $category->id])}}" id="editCategory">
	...
	<div class="col-lg-4">
		<div class="panel panel-primary">
	        ...
	        <div class="panel-body">
	        	...
	        	<form method="POST" action="{{route('store.album.subcategory', ['category' => $category->id])}}" id="addSubcategory">
       	            <div class="form-group">
...
<div class="row">
    <div class="col-lg-12">
        <hr>
        <h5><a href="{{route('albumi.index.kategorija')}}"><i class="fa fa-arrow-circle-right"></i>&nbsp;&nbsp;nazad...</a></h5>
    </div>
</div>

@include('partials.modals.modal-album-sub-delete')
